...
function OverflowItem({ citation, onClick }: OverflowItemProps) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="group hover:bg-muted focus-visible:bg-muted flex w-full cursor-pointer items-center gap-2.5 rounded-md px-2 py-2 text-start transition-colors focus-visible:outline-none"
    >
      {citation.favicon ? (
        <img
        ...
        <PopoverTrigger asChild>
          ...
          >
            <div className="flex items-center">
              ...
              {remainingCount > 0 && (
                <div
                  className="border-border bg-background dark:border-foreground/20 relative -ms-2 flex size-6 items-center justify-center rounded-full border shadow-xs"
                  style={{ zIndex: 0 }}
                >
